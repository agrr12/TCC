...
def save_df(path, file_name, save_mode, dataframe):
    create_path_if_not_exists(path)
    full_path = os.path.join(path, file_name)
    file_exists = os.path.exists(f'{full_path}.csv')
    df_with_time = add_timestamp(dataframe)
    df_with_time.to_csv(f'{full_path}.csv', index=False, mode=save_mode, header=not file_exists, encoding='utf-8')
